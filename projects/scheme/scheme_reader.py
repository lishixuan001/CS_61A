...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            return nil
            # END PROBLEM 1
        elif src.current() == '.': # If there is a dot, the next one should not be')''
            # BEGIN PROBLEM 2
            src.remove_front()
            nextitem = scheme_read(src) # Operate and delete the materials
            if src.current() != ")": # Check if closed '()'s
                raise SyntaxError('Invalid malformed list')
            src.remove_front()
            ''' Do not really understand why remoeve_front here before return a material values'''
            return nextitem
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
